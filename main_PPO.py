...
def save_params_to_json(params, filename_prefix="params", output_path='data'):
    ...
    # ========== 修复核心：改用绝对路径定位文件 ==========
    # 获取当前脚本（main_PPO.py）的目录
    current_script_dir = os.path.dirname(os.path.abspath(__file__))

    # 1. 复制 main_PPO.py（绝对路径）
    src_file = os.path.join(current_script_dir, 'main_PPO.py')
    dst_file = os.path.join(output_path, 'main_PPO.py')  # 用os.path.join兼容Windows/Linux路径
    if os.path.exists(src_file):
        shutil.copy2(src_file, dst_file)
    else:
        print(f"警告：未找到 {src_file}，跳过复制")

    # 2. 复制 PPO.py（绝对路径）
    src_file = os.path.join(current_script_dir, 'PPO.py')
    dst_file = os.path.join(output_path, 'PPO.py')
    if os.path.exists(src_file):
        shutil.copy2(src_file, dst_file)
    else:
        print(f"警告：未找到 {src_file}，跳过复制")
...
async def main(args):
    ...
    if x_values:
        ...
        plt.close("all")
        plt.figure(figsize=(8, 6))  # 增大画布尺寸
        plt.plot(x_values, y_values, marker="o", markersize=10, markerfacecolor='none', linestyle="-", color="b",
                 label='C')
        plt.plot(x_values, 1 - y_values, marker="d", markersize=10, markerfacecolor='none', linestyle="-", color="r",
                 label='D')
        # plt.title("Density")
